...
# Docker Desktop and some managed environments already provide binfmt/qemu support.
# Set SKIP_BINFMT_CHECK=1 to bypass host-side qemu/binfmt checks.
if [ "${SKIP_BINFMT_CHECK:-0}" = "1" ]; then
  binfmt_misc_required=0
fi
...
time ${DOCKER} run \
  ...
  pi-gen \
  bash -e -o pipefail -c "
    (dpkg-query -W -f='\${Status}' qemu-user-binfmt 2>/dev/null | grep -q 'install ok installed' && dpkg-reconfigure qemu-user-binfmt || true) &&
